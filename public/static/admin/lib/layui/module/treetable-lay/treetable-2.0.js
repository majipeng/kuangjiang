layui.define(['layer', 'table'], function (exports) {
  ...
  var treetable = {
    ...
    childs_checkbox: function (pid, is_checked) {
      var $dom = $(treetable.params.elem).next('.treeTable');
      // console.log($dom.find('.layui-table-body tbody [lay-tpid='+pid+']'));
      $dom.find('.layui-table-body tbody [lay-tpid=' + pid + ']').each(function () {
        var d = $(this).parents('tr');
        var checkbox = d.find('.laytable-cell-checkbox .layui-form-checkbox');
        var checkbox_input = d.find('.laytable-cell-checkbox input');
        var index = d.data('index');

        var cache_table = table.cache.table[index];
        if ('LAY_CHECKED' in cache_table) {
          delete cache_table.LAY_CHECKED;
          checkbox_input.removeAttr('checked');
        } else {
          cache_table.LAY_CHECKED = true;
          checkbox_input.attr('checked', true);
        }
        is_checked ? checkbox.addClass('layui-form-checked') : checkbox.removeClass('layui-form-checked');
        treetable.childs_checkbox(d.find('span').attr('lay-tid'), is_checked);
      });
    },
    childs_hide: function () {
    ...
    checked: function (dom) {
      var ids = [];
      $(dom).next('.treeTable').find('.layui-table-body tbody .layui-form-checked').each(function () {
        var id = $(this).parents('[data-content]').data('content');
        ids.push(id);
      });
      return ids;
    },
